  ...
  return (
    <div className="flex items-center justify-center">
      <ResponsiveDialog>
        ...
        <ResponsiveDialogContent className="w-[calc(100vw-1rem)] max-w-full max-h-[90vh] min-w-0 overflow-x-hidden p-0 md:max-w-4xl md:h-[85vh] md:!flex md:flex-col md:overflow-hidden md:p-0 md:gap-0">
          {/* Sticky header */}
          <div className="shrink-0 min-w-0 border-b border-border bg-background/80 px-4 py-4 backdrop-blur-sm sm:px-6 sm:py-5 md:px-8">
            <div className="flex min-w-0 flex-wrap items-center justify-between gap-3 sm:gap-4">
              <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-4">
                <h4 className="min-w-0 max-w-full break-words font-display text-xl font-bold leading-tight tracking-tight text-foreground md:text-2xl">
                  {project.title}
                </h4>
                <span className="max-w-full break-words rounded-full border border-border px-3 py-0.5 text-[11px] uppercase tracking-widest text-muted-foreground">
                  {project.category}
                </span>
              </div>
          ...
          {/* Scrollable content */}
          <ScrollArea className="min-w-0 max-w-full flex-1 overflow-x-hidden" type="always" data-lenis-prevent>
            <div className="min-w-0 max-w-full overflow-x-hidden px-4 py-5 sm:px-6 sm:py-6 md:px-8 md:py-8">
              {/* Tech stack */}
              <motion.div
                initial={{ opacity: 0, y: 12 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: 0.1 }}
                className="mb-8 flex min-w-0 max-w-full flex-col gap-5 overflow-hidden md:mb-10 md:flex-row md:gap-10"
              >
                {project.skills.frontend?.length > 0 && (
                  <div className="flex min-w-0 max-w-full flex-col items-center gap-2 md:items-start">
                    ...
                    <FloatingDock items={project.skills.frontend} desktopClassName="max-w-full flex-wrap h-auto min-h-16" />
                  </div>
                )}
                {project.skills.backend?.length > 0 && (
                  <div className="flex min-w-0 max-w-full flex-col items-center gap-2 md:items-start">
                    ...
                    <FloatingDock items={project.skills.backend} desktopClassName="max-w-full flex-wrap h-auto min-h-16" />
                  </div>
                )}
              </motion.div>
