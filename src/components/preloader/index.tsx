...
function Preloader({ children, disabled = false }: PreloaderProps) {
  ...
  const bypassLoading = () => {
    ...
    safetyTimeout.current && clearTimeout(safetyTimeout.current);
  ...
  useEffect(() => {
    ...
    // Keep dismissal independent of the GSAP ticker. A stalled animation must
    // never leave the portfolio content hidden behind the splash.
    safetyTimeout.current = setTimeout(() => {
      setLoadingPercent(100);
      setIsLoading(false);
    }, LOADING_TIME * 1000 + 500);
    return () => {
      loadingTween.current?.kill();
      safetyTimeout.current && clearTimeout(safetyTimeout.current);
    };
